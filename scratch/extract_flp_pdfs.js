    ...
    try {
        ...
        while (retries > 0) {
            try {
                ...
                if (err.message.includes('503') || err.message.includes('Service Unavailable') || err.message.includes('429')) {
                    console.log(`⚠️ API returned ${err.message}. Retrying in ${backoff/1000} seconds... (${retries} retries left)`);
                    await delay(backoff);
                    retries--;
                    backoff *= 2;
                } else {
        ...
        if (!result) {
            throw new Error('Failed after multiple retries due to server being overloaded.');
        }
...
async function main() {
    ...
    const files = fs.readdirSync(PDF_DIR).filter(f => f.toLowerCase().endsWith('.pdf') && !f.toLowerCase().includes('kips'));
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        if (fs.existsSync(path.join(OUTPUT_DIR, file + '.json'))) {
