...
export default function MockTestSession() {
  ...
  useEffect(() => {
    async function loadQuestions() {
      const cached = sessionStorage.getItem('nat_active_mock_session');
      if (cached) {
        try {
          const parsed = JSON.parse(cached);
          if (!parsed.finished && parsed.group === user?.nat_group) {
      ...
      const all = await getMockTestQuestions(user?.nat_group);
      setAllQuestions(all);
    }
    loadQuestions();
  ...
  useEffect(() => {
    ...
    if (finished) {
      sessionStorage.removeItem('nat_active_mock_session');
      return;
    }

    const stateObj = {
      group: user?.nat_group,
      allQuestions,
      currentIndex,
      answers,
      timeLeft,
    ...
    sessionStorage.setItem('nat_active_mock_session', JSON.stringify(stateObj));
  }, [user?.nat_group, allQuestions, currentIndex, answers, markedQuestions, finished]);
  ...
  useEffect(() => {
    ...
    if (cached) {
      try {
        ...
        sessionStorage.setItem('nat_active_mock_session', JSON.stringify(parsed));
      } catch(e) {}
    }
  }, [timeLeft, allQuestions.length, finished]);
  ...
  if (finished && showReview) {
    return (
      <div className="p-6 max-w-4xl mx-auto space-y-6 bg-background">
        ...
        <button onClick={() => navigate('/')} className="w-full btn-primary py-4 rounded-xl shadow-lg transition">
          Go Home
        </button>
      </div>
    );
    ...
    return (
      <div className="p-6 max-w-4xl mx-auto space-y-6 bg-background">
        <div className="bg-card rounded-3xl border border-border p-8 text-center shadow-sm">
          ...
          <p className="text-muted-foreground mb-8">{user?.nat_group} Group • 90 Questions</p>
      ...
      <div className="sticky top-0 z-20 bg-background/80 backdrop-blur-md border-b border-border">
        <div className="flex flex-wrap items-center justify-between px-4 sm:px-6 py-4 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="font-display text-xl font-bold text-foreground">NAT Mock Test</h2>
              <p className="text-xs text-muted-foreground">Official Simulation • {user?.nat_group}</p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-3 sm:gap-4">
            ...
            <button onClick={() => { sessionStorage.removeItem('nat_active_mock_session'); navigate('/'); }} className="flex items-center gap-2 text-sm font-semibold text-muted-foreground border border-border bg-card px-3 sm:px-4 py-2 rounded-lg hover:bg-secondary transition">
              Quit
            </button>
          </div>
        </div>
