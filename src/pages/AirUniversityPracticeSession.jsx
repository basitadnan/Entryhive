...
export default function AirUniversityPracticeSession() {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        try {
          const insertPromise = supabase.from('AirUniversityPracticeSession').insert({
          ...
          const timeoutPromise = new Promise((resolve) => 
            setTimeout(() => resolve({ data: null, error: { message: 'Insert timed out' } }), 2000)
          );
          ...
          if (!error && record) {
            ...
            if (error) console.warn('Supabase session insert error/timeout, using local session ID:', error);
          }
        } catch (supabaseErr) {
          setSessionId(`local_session_${Date.now()}`);
    ...
    return (
      <div className="p-6 max-w-4xl mx-auto space-y-6 bg-background">
        ...
        <div className="flex gap-4 pt-4">
          <button onClick={() => navigate('/practice')} className="flex-1 bg-card border border-border text-foreground font-bold py-3.5 rounded-xl hover:bg-secondary transition">
            Back to Practice
          </button>
          <button onClick={() => navigate('/air-university')} className="flex-1 btn-primary py-3.5 rounded-xl shadow-lg transition">
            Go Home
          </button>
        </div>
      </div>
    ...
    <div className="flex flex-col flex-1 bg-background">
      ...
      <div className="sticky top-0 z-20 bg-background/80 backdrop-blur-md border-b border-border">
        <div className="flex flex-wrap items-center justify-between px-4 sm:px-6 py-4 gap-4">
          <div>
            <h2 className="font-display text-xl font-bold text-foreground">Air University - {getSectionLabel(section)}</h2>
            <p className="text-xs text-muted-foreground">AU Practice Session • {subTopic || difficulty}</p>
          </div>
          <div className="flex flex-wrap items-center gap-3 sm:gap-4">
            ...
            <button onClick={() => { sessionStorage.removeItem('active_practice_session'); navigate('/air-university'); }} className="flex items-center gap-2 text-sm font-semibold text-[hsl(var(--accent))] border border-[hsl(var(--accent))]/20 bg-[hsl(var(--accent))]/10 px-3 sm:px-4 py-2 rounded-lg hover:opacity-80 transition">
