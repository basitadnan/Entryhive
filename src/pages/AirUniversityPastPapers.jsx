...
import { 
  ...
  BrainCircuit, Calculator, Languages, Microscope, Monitor,
...
const categoryIcons = {
  ...
  'Analytical': <BrainCircuit className="w-8 h-8 text-pink-500" />,
  'Quantitative': <Calculator className="w-8 h-8 text-emerald-500" />,
  'Physics': <Microscope className="w-8 h-8 text-amber-500" />,
  'Mathematics': <Calculator className="w-8 h-8 text-cyan-500" />,
  'Computer Science': <Monitor className="w-8 h-8 text-violet-500" />,
  'Chemistry': <Microscope className="w-8 h-8 text-rose-500" />,
  'Biology': <Microscope className="w-8 h-8 text-green-500" />
};

const sectionColors = {
  'English': 'from-blue-500/20 to-blue-500/5 border-blue-500/30 text-blue-500 hover:border-blue-500/50',
  'Analytical': 'from-pink-500/20 to-pink-500/5 border-pink-500/30 text-pink-500 hover:border-pink-500/50',
  'Quantitative': 'from-emerald-500/20 to-emerald-500/5 border-emerald-500/30 text-emerald-500 hover:border-emerald-500/50',
  'Physics': 'from-amber-500/20 to-amber-500/5 border-amber-500/30 text-amber-500 hover:border-amber-500/50',
  'Mathematics': 'from-cyan-500/20 to-cyan-500/5 border-cyan-500/30 text-cyan-500 hover:border-cyan-500/50',
  'Computer Science': 'from-violet-500/20 to-violet-500/5 border-violet-500/30 text-violet-500 hover:border-violet-500/50',
  'Chemistry': 'from-rose-500/20 to-rose-500/5 border-rose-500/30 text-rose-500 hover:border-rose-500/50',
  'Biology': 'from-green-500/20 to-green-500/5 border-green-500/30 text-green-500 hover:border-green-500/50',
};
...
export default function AirUniversityPastPapers() {
  ...
  useEffect(() => {
    async function fetchAllAUPapers() {
      try {
        setLoading(true);
        const eng = await getQuestions('english', 'all', 200);
        const analytical = await getQuestions('analytical', 'all', 200);
        const quantitative = await getQuestions('quantitative', 'all', 200);
        const physics = await getQuestions('physics', 'all', 200);
        const chemistry = await getQuestions('chemistry', 'all', 200);
        const mathematics = await getQuestions('mathematics', 'all', 200);
        const biology = await getQuestions('biology', 'all', 200);
        const computer_science = await getQuestions('computer_science', 'all', 200);

        const formattedEng = eng.map(q => ({...q, section: 'English'}));
        const formattedAnalytical = analytical.map(q => ({...q, section: 'Analytical'}));
        const formattedQuantitative = quantitative.map(q => ({...q, section: 'Quantitative'}));
        const formattedPhysics = physics.map(q => ({...q, section: 'Physics'}));
        const formattedChemistry = chemistry.map(q => ({...q, section: 'Chemistry'}));
        const formattedMathematics = mathematics.map(q => ({...q, section: 'Mathematics'}));
        const formattedBiology = biology.map(q => ({...q, section: 'Biology'}));
        const formattedCS = computer_science.map(q => ({...q, section: 'Computer Science'}));

        setQuestions([
          ...formattedEng, 
          ...formattedAnalytical, 
          ...formattedQuantitative, 
          ...formattedPhysics,
          ...formattedChemistry,
          ...formattedMathematics,
          ...formattedBiology,
          ...formattedCS
        ]);
      } catch (error) {
        console.error('Failed to load AU past papers:', error);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (!user?.is_premium) {
    return (
      <div className="p-6 max-w-3xl mx-auto space-y-8 pb-24">
        <button onClick={() => navigate('/air-university')} className="text-sm text-muted-foreground flex items-center gap-1 hover:text-foreground transition-colors font-medium">
          <ArrowLeft className="w-4 h-4" /> Back to AU Hub
        </button>
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <div className="space-y-4 relative z-10">
            ...
            <p className="text-base text-muted-foreground max-w-sm mx-auto leading-relaxed font-medium">
              Unlock over 1,600+ real Air University past paper questions categorized by subject.
            </p>
          </div>
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <h1 className="font-display text-4xl font-black text-foreground">Air University Past Papers</h1>
          ...
          <p className="text-muted-foreground font-medium text-lg">Master the Air University Entry Test by practicing with real previous exam questions.</p>
        </div>
        <button onClick={() => navigate('/air-university')} className="py-2.5 px-5 bg-card border-2 border-border text-foreground hover:bg-secondary rounded-xl font-bold transition-colors shadow-sm self-start sm:self-center flex items-center gap-2">
          <ArrowLeft className="w-4 h-4" /> AU Hub
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="p-16 text-center border-4 border-dashed border-border bg-card/50 rounded-[40px]">
          ...
          <p className="text-base text-muted-foreground max-w-sm mx-auto mb-8 font-medium">No Air University past paper questions have been added to the database yet.</p>
        </div>
      ) : (
          ...
          {categories.map((cat, i) => (
            ...
            >
              <div 
                ...
                onClick={() => setActivePaper({ title: `Air University ${cat} - Past Papers`, questions: grouped[cat] })}
              >
                ...
                <div className="space-y-2 relative z-10">
                  ...
                  <p className="text-sm font-medium text-muted-foreground">Air University previous exam bank</p>
                </div>
