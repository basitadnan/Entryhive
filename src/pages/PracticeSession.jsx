  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        try {
          const insertPromise = supabase.from('PracticeSession').insert({
    ...
    return (
      <div className="p-6 max-w-4xl mx-auto space-y-6 bg-background">
        ...
        <div className="flex gap-4 pt-4">
          ...
          <button onClick={() => navigate('/')} className="flex-1 btn-primary py-3.5 rounded-xl shadow-lg transition">
            Go Home
          </button>
        </div>
      </div>
    ...
    <div className="flex flex-col flex-1 bg-background">
      ...
      <div className="sticky top-0 z-20 bg-background/80 backdrop-blur-md border-b border-border">
        <div className="flex flex-wrap items-center justify-between px-4 sm:px-6 py-4 gap-4">
          <div>
            <h2 className="font-display text-xl font-bold text-foreground">{getSectionLabel(section)}</h2>
            <p className="text-xs text-muted-foreground">Practice Session • {subTopic || difficulty}</p>
          </div>
          <div className="flex flex-wrap items-center gap-3 sm:gap-4">
            ...
            <button onClick={() => { sessionStorage.removeItem('active_practice_session'); navigate('/practice'); }} className="flex items-center gap-2 text-sm font-semibold text-[hsl(var(--accent))] border border-[hsl(var(--accent))]/20 bg-[hsl(var(--accent))]/10 px-3 sm:px-4 py-2 rounded-lg hover:opacity-80 transition">
